import pygame
import os, copy, math
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.utils import generate_temporary_file_path, classproperty, cumulative_rewards_from_rew_list
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.static import GRAPHICS_DIR, FONTS_DIR
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.mdp.layout_generator import EMPTY, COUNTER, ONION_DISPENSER, TOMATO_DISPENSER, POT, DISH_DISPENSER, SERVING_LOC
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.visualization.visualization_utils import show_image_in_ipython, show_ipython_images_slider
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.visualization.pygame_utils import MultiFramePygameImage, run_static_resizeable_window, vstack_surfaces, scale_surface_by_factor, blit_on_new_surface_of_size
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.mdp.actions import Direction, Action

roboto_path = os.path.join(FONTS_DIR, "Roboto-Regular.ttf")


class StateVisualizer:
    TERRAINS_IMG = OBJECTS_IMG = SOUPS_IMG = CHEFS_IMG = None
    ARROW_IMG = INTERACT_IMG = STAY_IMG = None
    ...
    def _load_images(cls):
        """Load optional renderer assets only when visualization is requested."""
        if cls.TERRAINS_IMG is not None:
            return
        required = ("terrain", "objects", "soups", "chefs")
        missing = [name for name in required if not os.path.exists(os.path.join(GRAPHICS_DIR, name + ".png"))]
        if missing:
            raise RuntimeError(
                "state visualization assets are absent from this checkout: "
                + ", ".join(missing)
            )
        cls.TERRAINS_IMG, cls.OBJECTS_IMG, cls.SOUPS_IMG, cls.CHEFS_IMG = [
            MultiFramePygameImage(
                os.path.join(GRAPHICS_DIR, name + ".png"),
                os.path.join(GRAPHICS_DIR, name + ".json"),
            )
            for name in required
        ]
        cls.ARROW_IMG = pygame.image.load(os.path.join(GRAPHICS_DIR, "arrow.png"))
        cls.INTERACT_IMG = pygame.image.load(os.path.join(GRAPHICS_DIR, "interact.png"))
        cls.STAY_IMG = pygame.image.load(os.path.join(GRAPHICS_DIR, "stay.png"))
