from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.utils import load_dict_from_file
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.data.layouts import LAYOUTS_DIR, read_layout_dict
...
try:
    from dtw import dtw
except ModuleNotFoundError:  # Dynamic-time-warping metrics use the full environment.
    dtw = None
import bisect
from collections import defaultdict

from datetime import datetime
from multimodal_comms.apps.collab_overcooked.settings import PROMPT_DIR, require_openai_key
...
REFERENCE_DIR = str(PROMPT_DIR / "reference")
...
class Evaluation:
    ...
    def dtw_similarity(self, reference, predict):
        ...
        if len(reference) == 0 or len(predict) == 0:
            return None, 0
        if dtw is None:
            raise RuntimeError("dynamic-time-warping metrics require the full Conda environment")
        dtw_result = dtw(
            predict,
            reference,
            keep_internals=True,
