...
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.mdp.actions import Action, Direction
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.planning.search import find_path
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.planning.search import get_intersect_counter
from multimodal_comms.apps.collab_overcooked.overcooked_ai_py.planning.search import query_counter_states
...
from multimodal_comms.apps.collab_overcooked.collab.modules import if_two_sentence_similar_meaning
from multimodal_comms.apps.collab_overcooked.settings import PROMPT_DIR, openai_keys
...
class LLMPair(object):
    ...
    def load_openai_keys(self):
        ...
        if not self.openai_api_keys:
            raise RuntimeError("set OPENAI_API_KEY before constructing an LLM agent")
...
- 2. Follow the Chef’s instructions unless they are incorrect. For example, if the Chef requests a utensil that is not available on your side, you should refuse and inform him. """
    ...
    def action(self, state):
        ...
        if getattr(self, "channel", None) is not None:
            self.channel.maybe_reset_for_timestep()
    ...
    def _record_talk(self, content: str) -> None:
        """Centralized store point for every outbound utterance.

        Keeps the original raw dialog_history_list (used by the self-repeat
        check) untouched, and additionally mirrors the utterance into the
        shared channel adapter's transcript when one is attached (it is not
        attached at all for runs of the original, unmodified protocol).
        """
        self.planner.dialog_history_list.append({"role": "talk", "content": content})
        if getattr(self, "channel", None) is not None:
            self.channel.record(self.agent_index, content)

    def message_formate_control(
        self, role: str, turn_dialog_self: list, turn_dialog_team: list
    ):
        # Channel-compression dispatch: with no adapter attached, or the
        # identity compressor selected, fall through to the exact original
        # (byte-identical) formatting logic below. Any other compressor
        # renders from the adapter's own chronological transcript instead.
        channel = getattr(self, "channel", None)
        if channel is not None:
            def name_for(agent_id):
                return self.name if agent_id == self.agent_index else self.teammate.name
            # Always run render() for its stats side-effect (transmitted_chars
            # must accumulate over every render call, same basis for every
            # condition including identity) even though identity's actual
            # prompt content below stays on the untouched legacy path.
            rendered = channel.render(self.agent_index, name_for)
            if channel.compressor_name != "identity":
                return rendered
        return self._message_formate_control_legacy(
            role, turn_dialog_self, turn_dialog_team
        )

    def _message_formate_control_legacy(
        self, role: str, turn_dialog_self: list, turn_dialog_team: list
    ):
        pre_message = ""
        answer_num = 1
    ...
    def parse_response(self, response, mode, need_correct=False):
        if self.agent_index == 0:
            ...
            role = "Assistant"
        if mode == "analysis":
            pattern = rf"(?:{role})\s+analysis\s*:?\s*(.*?)\s*(?:{role})\s+plan:"
    ...
    def generate_ml_action(self, state):
        ...
        if self.action_wait_parse.empty() or (not self.trace):
            ...
            if failure_message != "":
                ...
                self._record_talk(communicate_response)
                # statistic
                self.turn_statistics_dict["statistical_data"]["communication"][
                    self.agent_index
