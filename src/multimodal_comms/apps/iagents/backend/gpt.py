...
# Allows query_gpt4() to target a non-OpenAI model id (e.g. "deepseek-chat") when BASE_URL
# points at an OpenAI-compatible third-party endpoint instead of api.openai.com.
GPT4_MODEL_NAME = global_config.get("backend").get("gpt4_model_name", "gpt-4o-mini")
...
def calc_max_token(messages, model):
    ...
    try:
        ...
        # tiktoken has no tokeniser entry for third-party model ids (e.g. deepseek-*);
        # cl100k_base is a reasonable approximation for token-budget purposes here.
    ...
    num_max_token = num_max_token_map.get(model, 32768)
