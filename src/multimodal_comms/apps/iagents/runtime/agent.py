...
from multimodal_comms.apps.iagents.backend.gemini import query_gemini
from multimodal_comms.apps.iagents.backend.gpt import query_claude, query_gpt, query_gpt4
from multimodal_comms.apps.iagents.backend.third_party import *
from multimodal_comms.apps.iagents.runtime.tool import FaissTool, JsonFormatTool, MindFillTool, SqlTool
from multimodal_comms.apps.iagents.runtime.util import iAgentsLogger
from multimodal_comms.apps.iagents.runtime.llamaindex import LlamaIndexer
...
class Agent(ABC):
    ...
    def _get_query_func(self, backend: str):
        ...
        if backend == "gemini":
            ...
            from multimodal_comms.apps.iagents.backend.ollama import query_ollama
            return query_ollama
...
class MemoryAgent(ThinkAgent):
    ...
    def __init__(self, master: str, backend: str, task: str, is_assistant: bool = False, enable_distinct_memory: bool = True, enable_fuzzy_memory: bool = False, memory_name: str = "") -> None:
        ...
        self.stopwords = set()
        with open(os.path.join(file_path, "stopwords.txt"), "r") as f:
            for line in f:
